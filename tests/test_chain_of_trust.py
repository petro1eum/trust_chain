...
class TestVerifyChain:
    ...
    def test_verify_long_chain(self, tc):
        ...
        for i in range(10):
            step = tc._signer.sign(f"step_{i}", {"index": i}, parent_signature=parent_sig)
            chain.append(step)
    ...
    def test_verify_broken_chain(self, tc):
        step1 = tc._signer.sign("step1", {"data": 1})
        step2 = tc._signer.sign("step2", {"data": 2}, parent_signature=step1.signature)
        step3 = tc._signer.sign("step3", {"data": 3}, parent_signature="WRONG_SIGNATURE")
    ...
    def test_verify_chain_missing_first_parent(self, tc):
        ...
        step1_with_parent = tc._signer.sign("step1", {"data": 1}, parent_signature="fake")
        step2_linked = tc._signer.sign("step2", {"data": 2}, parent_signature=step1_with_parent.signature)
...
class TestChainOfTrustVerification:
    ...
    def test_chain_integrity(self, tc):
        ...
        step2 = fresh_tc._signer.sign("step2", {"data": 2}, parent_signature=step1.signature)
        step3 = fresh_tc._signer.sign("step3", {"data": 3}, parent_signature=step2.signature)
...
class TestRealWorldChainScenarios:
    ...
    def test_banking_workflow(self, tc):
        ...
        # Step 1: Authenticate
        auth = tc._signer.sign("authenticate", {
            "user": "alice",
            "method": "2fa",
            "verified": True
        })
        ...
        balance = tc._signer.sign("check_balance", {
            "account": "ACC001",
            "balance": 10000.0
        }, parent_signature=auth.signature)
        
        # Step 3: Execute transfer
        transfer = tc._signer.sign("transfer", {
            "from": "ACC001",
            "to": "ACC002",
            "amount": 500.0,
            "status": "completed"
        ...
        # Step 4: Confirm
        confirm = tc._signer.sign("confirm", {
    ...
    def test_ai_agent_workflow(self, tc):
        ...
        # Step 1: Search
        search = tc._signer.sign("search", {
            "query": "weather Moscow",
            "results": ["sunny", "22C"]
        })
        
        # Step 2: Analyze
        analyze = tc._signer.sign("analyze", {
            "input_summary": "sunny 22C",
            "conclusion": "Good weather"
        }, parent_signature=search.signature)
        
        # Step 3: Generate response
        response = tc._signer.sign("generate_response", {
