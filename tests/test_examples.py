...
class TestExamplesRun:
    ...
    def test_mcp_claude_desktop_example(self):
        ...
        import_result = subprocess.run(
            [sys.executable, "-c", "import sys; sys.path.insert(0, '.'); exec(open('examples/mcp_claude_desktop.py').read().split('if __name__')[0])"],
            capture_output=True,
            text=True,
            timeout=10,
            cwd=PROJECT_ROOT
        ...
        # Import should work
        assert import_result.returncode == 0, f"mcp_claude_desktop.py import failed:\n{import_result.stderr}"
    ...
    def test_full_enforcement_demo(self):
        ...
        assert result.returncode == 0, f"full_enforcement_demo.py failed:\n{result.stderr}"
...
class TestExamplesNoRegressions:
    ...
    def test_no_nonce_replay_in_verification_demo(self):
        ...
        assert result.returncode == 0, f"Nonce replay regression! Error:\n{result.stderr}"
        assert "NonceReplayError" not in result.stderr, "NonceReplayError should not occur"
