...
| **Merkle verifiable log** | Append-only, tamper-evident chain log (Postgres backend optional). `tc chain-verify --pubkey` re-verifies every Ed25519 signature. |
...
| **HTML/PDF audit exports** | One-click beautiful reports for auditors, rendered over the OSS Merkle log. Show compliance without technical knowledge. |
...
| **Merkle verifiable log** | Append-only, защищённый от подделки лог цепочки (опционально на Postgres). `tc chain-verify --pubkey` перепроверяет каждую Ed25519-подпись. |
...
| **HTML/PDF экспорт аудита** | Красивые отчёты для аудиторов одним кликом, поверх OSS Merkle-лога. Показать соответствие требованиям без технических знаний. |
