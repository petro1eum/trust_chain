...
```
При запуске `tc.chain.verify(public_key=pk)` TrustChain пройдёт по всему графу (DAG) и криптографически перепроверит **каждую** подпись, включая родительские ссылки (они входят в подписанный payload, поэтому переписать родителя незаметно нельзя). **Без** ключа проверяется только наличие связей в графе, не криптография.
...
| Команда TrustChain | Аналог в Git | Описание и примеры |
|--------------------|--------------|-------------------|
| `tc chain-verify` | `git fsck` | **Самая важная команда.** Всегда проверяет целостность связей DAG (каждая родительская ссылка существует в цепи). С публичным ключом (`tc chain-verify --pubkey key.json`) дополнительно математически перепроверяет **каждую** подпись Ed25519 — и тогда обнаруживает ручную подделку payload или родительских ссылок. Без ключа — только структура. |
